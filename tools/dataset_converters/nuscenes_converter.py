...
def create_nuscenes_infos(root_path,
    ...
    Args:
        ...
        out_path (str): Path of the data info to save.
        can_bus_root_path (str): Path of the canbus data root.
    ...
    assert version in available_vers, f"{version} is not available"
    logger.info(f"create nuscenes infos")
    logger.info(f"version: {version}, root_path: {root_path}, info_save_path: {out_path}")
    ...
    if test:
        ...
        logger.info(f"save test info to {info_path}")
        ...
        logger.info(f"save train info to {info_path}")

        mmengine.dump(data, info_path)
        data['infos'] = val_nusc_infos
        info_val_path = osp.join(out_path, '{}_infos_val.pkl'.format(info_prefix))
        logger.info(f"save info val to {info_val_path}")
    ...
    for scene in nusc.scene:
        ...
        while has_more_frames:
            ...
            if os.getcwd() in lidar_path:
                # The nuscenes data stores relative paths, 
                # so it will not enter this if 
...
def _get_can_bus_info(nusc, nusc_can_bus, sample):
    ...
    # last_pose include key: accel、orientation、pos、rotation_rate、utime、vel
    # accel: the current frame self-vehicle acceleration (3, m/s/s), 
    # orientation: the current frame is facing from the vehicle (4), 
    # pos: the position of the car in the world coordinate system (3, m), 
    #       equal to the ego pose, but with a faster frequency
    # rotation_rate: the current frame is from the angular velocity of the car (3, rad/s)
    # utime: timestamp
    # vel: the current frame self-vehicle speed (3, m/s)

    # find the most recent CAN data before the current timestamp
    ...
    can_bus.extend([0., 0.])
    # [x, y, z, ori_w, ori_x, ori_y, ori_z, ax, ay, az, w_x, w_y, w_z, vx, vy, vz, 0, 0]
    # ori_: quaternion,     a_: acceleration,     w_: angular velocity,     v_: speed
...
def _fill_trainval_infos(nusc,
    ...
    Args:
        ...
        nusc_can_bus (:obj:`NuScenes`): Dataset class in the nuScenes Canbus dataset.
    ...
    for sample in mmengine.track_iter_progress(nusc.sample):
        ...
        # Corresponds to the box under the sensor
        ...
        info = {
            ...
            'lidar2ego_rotation': np.array(cs_record['rotation']),  # quaternion
        ...
        if sample['next'] == '':
            # Since the value is assigned first and then added by one, 
            # the id should be assigned a value of 0 at the last frame of the previous scene
            frame_idx = 0
        else:
            frame_idx += 1
        ...
        for cam in camera_types:
            ...
            cam_intrinsic = get_cam_intrinsic(nusc, cam_token)  # accelerate purpose
        ...
        for radar_name in radar_types:
            ...
            radar_info['num_features'] = 18
            info['radars'].update({radar_name: radar_info})

            while len(sweeps) < max_sweeps:
                if not radar_rec['prev'] == '':
                    radar_sweep = obtain_sensor2top(nusc, radar_rec['prev'], l2e_t, l2e_r_mat,
                                                   e2g_t, e2g_r_mat, radar_name)
                    radar_sweep['num_features'] = 18
                    # if 'samples' in radar_sweep['data_path']:  # change to only save non-key frames
                    #     break
                    sweeps.append(radar_sweep)
                    radar_rec = nusc.get('sample_data', radar_rec['prev'])
                else:
                    break
            info['radar_sweeps'].update({radar_name: sweeps})

        # obtain sweeps for a single key-frame
        sd_rec = nusc.get('sample_data', sample['data']['LIDAR_TOP'])
        sweeps = []
        while len(sweeps) < max_sweeps:
            # will save the key-frame data in the sweeps
            if not sd_rec['prev'] == '':
                sweep = obtain_sensor2top(nusc, sd_rec['prev'], l2e_t,
                                          l2e_r_mat, e2g_t, e2g_r_mat, 'lidar')
                sweep['num_features'] = 5
                # if 'samples' in sweep['data_path']:  # change to only save non-key frames
                #     break
                sweeps.append(sweep)
                sd_rec = nusc.get('sample_data', sd_rec['prev'])
            else:
                break
        info['sweeps'] = sweeps  # Lidar non-keyframe information
        ...
        if not test:
            ...
            # nus_track_categories = ['car', 'truck', 'trailer', 'bus','bicycle', 'motorcycle', 'pedestrian']
            # name_in_track = [name in nus_track_categories for name in names]
            # name_in_track = np.array(name_in_track)
            # track_valid_flag = np.logical_and(valid_flag, name_in_track)
            # info['instance_inds'] = instance_inds
            # info['track_valid_flag'] = track_valid_flag
